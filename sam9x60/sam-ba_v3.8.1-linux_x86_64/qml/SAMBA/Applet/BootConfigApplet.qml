...
Applet {
	...
	/*! \internal */
	function callReadBootCfg(index, option)
	{
		var status, cmd
		...
		if (cmd) {
	       if (option === undefined)
	           option = 0
			status = connection.appletExecute(cmd, [index, option])
			if (status !== 0)
	...
	*/
	...
	{
		return callReadBootCfg(index, option)
	}
	...
	*/
	...
	function commandLineCommandReadBootConfig(args) {
		if (args.length !== 1 && args.length !== 2 )
			return "Invalid number of arguments (expected 1 or 4)."
        ...
        if (args.length === 2) {
            ...
            var version_pkt_info = configValueFromText(index, args[1])
            if (isNaN(version_pkt_info) && typeof version_pkt_info === "undefined") {
                return "Invalid value parameter"
            }
            nvm_info = version_pkt_info[1]
        }


		var value = readBootCfg(index, nvm_info)

...
Applet {
	...
	property string nandHeaderHelp: "For information on PMECC config, please refer to product datasheet."
	...
	function buildInitArgs() {
		...
		if (typeof config.header === "undefined")
			throw new Error("Incomplete NAND Flash configuration, missing value for 'pmecc_cfg' property")
		...
		if (args.length >= 3) {
			if (args[2].length > 0) {
				...
				if (isNaN(config.header))
					return "Invalid NAND PMECC config (not a number)."
			}
		}
	...
	function commandLineHelp() {
		return ["Syntax: nandflash:[<ioset>]:[<bus_width>]:[<pmecc_cfg>]",
		        "Parameters:",
		        "    ioset      I/O set",
		        ...
		        "    nandflash:2:8:0xc0098da5  use fully custom settings (IOSET2, 8-bit bus, pmecc config is 0xc0098da5)",
		        "    nandflash:::0xc0098da5    use default board settings but force pmecc config to 0xc0098da5",
		        nandHeaderHelp]
	}

...
Item {
	...
	*/
    ...
    /*! \internal */
    function read_Impl(offset, size, fileName, append)
    {
        if (!canReadPages())
            throw new Error("Applet '" + name +
                            "' does not support 'read pages' command")

        var file = File.open(fileName, true, append)
        if (!file)
    ...
    */
    ...
    {
       read_Impl(offset, size, fileName, false)
    }

    /*!
        \qmlmethod void Applet::read_and_append(int offset, int size, string fileName)
        \brief Read data from the device and append data into a file.
        ...
        error occured during reading
    */
    function read_and_append(offset, size, fileName)
    {
       read_Impl(offset, size, fileName, true)
    }
	...
	*/
	...
	function defaultCommandLineCommands() {
		...
		if (canReadPages()) {
			...
			if (device.bootstrap_image != true)
	...
	function defaultCommandLineCommandHelp(command) {
        ...
        else if (command === "writeboot" && device.bootstrap_image != true) {
		...
		else if (command === "verifyboot" && device.bootstrap_image != true) {
	...
	function defaultCommandLineCommand(command, args) {
        ...
        else if (command === "writeboot" && canWritePages() && device.bootstrap_image != true) {
		...
		else if (command === "verifyboot" && canReadPages() && device.bootstrap_image != true) {
